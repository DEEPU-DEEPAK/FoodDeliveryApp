...
export const createOrder = async (req, res) => {
    try {
        ...
        if (!items || !Array.isArray(items) || items.length === 0) {
            return res.status(400).json({ message: "invalid item array" })
        }
        ...
        const orderItems = items.map(({ item, name, price, imageUrl, quantity }) => {
            ...
            return {
                item: {
                    ...
                    price: Number(base.price || price || 0),  //price: Number(base.price ?? price) || 0,
...
//update the order by admin without token
