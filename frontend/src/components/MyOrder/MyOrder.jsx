...
const MyOrder = () => {
    ...
    if (error) {
        <div className="min-h-screen bg-gradient-to-br from-[#0D1B2A] to-[#1B263B] flex items-center justify-center text-red-400 text-xl gap-4">
            <p>{error}</p>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-[#0D1B2A] to-[#1B263B] text-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <div className="flex items-center gap-4">
                        <span className="text-cyan-400/70 text-sm">{user?.email}</span>
                    </div>
                </div>

                <div className="bg-[#1E2A38] backdrop-blur-sm rounded-3xl p-8 shadow-2xl border-2 border-cyan-500/20">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-[#2D3A4E]">
                                <tr>
                                    <th className="p-4 text-left text-cyan-400">Order ID</th>
                                    <th className="p-4 text-left text-cyan-400">Customer</th>
                                    <th className="p-4 text-left text-cyan-400">Address</th>
                                    <th className="p-4 text-left text-cyan-400">Items</th>
                                    <th className="p-4 text-left text-cyan-400">Total Items</th>
                                    <th className="p-4 text-left text-cyan-400">Price</th>
                                    <th className="p-4 text-left text-cyan-400">Payment</th>
                            ...
                            <tbody className="">
                                {
                                    orders.map((order) => {
                                        ...
                                        return (
                                            <tr key={order._id} className="border-b border-cyan-500/20 hover:bg-[#2D3A4E]/50 transition-colors group">
                                                <td className='p-4 text-cyan-100 font-mono text-sm'>#{order._id?.slice(-8)}</td>
                                                <td className='p-4'>
                                                    <div className="flex items-center gap-2">
                                                        <FiUser className="text-cyan-400" />
                                                        <div>
                                                            <p className="text-cyan-100">{order.firstName} {order.lastName} </p>
                                                            <p className="text-sm text-cyan-400/60">{order.phone}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                        ...
                                                        {order.items.map((item, index) => (
                                                            <div key={`${order._id}-${index}`} className="flex items-center gap-4 bg-[#2D3A4E] rounded-lg p-2 justify-center">
                                                                ...
                                                                <div className="flex-1">
                                                                    <span className="text-cyan-100/80 text-sm block">
                                                                        {item.item.name}
                                                                    </span>
                                                ...
                                                <td className="p-4 text-cyan-300 text-lg">₹{totalPrice.toFixed(2)}</td>
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`px-4 py-2 rounded-lg ${status.bg} ${status.color} border border-cyan-500/20 text-sm`}>
                                                            {status.label}</span>
                                                    </div>
                                                </td>
                                            </tr>
                    ...
                    {orders.length === 0 && (
                        <div className="text-center py-12 text-cyan-100/60 text-xl">No Orders Found</div>
                    )}
                </div>
            </div>
        </div>
